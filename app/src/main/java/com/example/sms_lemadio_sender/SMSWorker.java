...
/*public class SMSWorker extends Worker {
    ...
    public Result doWork() {
        ...
        long MAX_EXECUTION_TIME = 9_000; // 9 secondes
        ...
        try {
            while (System.currentTimeMillis() - startTime < MAX_EXECUTION_TIME) {
                ...
                if (!unsentClients.isEmpty()) {
                    ...
                    unsentClients.parallelStream()
                            .limit(5) // Limiter à 5 clients par cycle
                            .forEach(this::envoyerSMS);
                } else {
                    break;
                }
            }
    ...
    private List<ClientInfo> recupererClientsNonTraites() {
        ...
        try { URL url = new URL(ApiUrl.API_URL);
    ...
    private void envoyerSMS(ClientInfo client) {
        try { String numero = formaterNumero(client.phoneNumber);
            if (!validerNumeroMadagascar(numero)) {
                Log.e(TAG, "Numéro invalide : " + numero);
                mettreAJourStatutSMS(client.stove); return;
            }
            String message = String.format( "Bonjour %s! Nous vous remercions pour votre achat chez ADES. Vous avez acquis un réchaud modèle %s. Votre numéro de facture est : %s. Pour consulter les détails de votre garantie, veuillez cliquer sur ce lien : %s. Nous vous remercions pour votre confiance!",
                    client.name,
                    client.stove,
            ...
            mettreAJourStatutSMS(client.stove);
        } catch (Exception e) {
    ...
    private void mettreAJourStatutSMS(String stove) {
        try {
            ...
            if (responseCode >= 200 && responseCode <= 299) {
                ...
                Log.e(TAG, "Échec mise à jour statut SMS: " + responseCode);
            }
        } catch (Exception e) {
                    ...
                    if ("success".equals(jsonResponse.getString("status"))) {
                        ...
                        for (int i = 0; i < clientsArray.length(); i++) {
                            ...
                            if (!clientJson.optBoolean("is_sms_sent", false)) {
                                ...
                                client.consultation = clientJson.getString("consultation_url");
                                unsentClients.add(client);
                                Log.d("List", "Nom: "+client.name+"\nTel: "+client.phoneNumber+" smsSent: "+client.smsSent);
                            }
                        }
                    }
                }
